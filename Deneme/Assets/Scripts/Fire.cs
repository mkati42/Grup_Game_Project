...
public class Fire : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if ((gunsIndex == 1 && fireTime > 1.0f) || (gunsIndex == 2 && fireTime > 20.0f))
            {
                _damage = playerStat.getDamage(gunsIndex);
                if (playerStat._energy >= 100f && gunsIndex == 2 && Input.GetKey(KeyCode.LeftControl))
                {
                    ...
                    shake.StartShakeCoroutine(0.20f, 2f);
                    shoot();
                    voice.stopSelectedVoice("fire");
    ...
    private void shoot()
    {
        ...
        tempAmmo.GetComponent<Rigidbody2D>().AddForce((target.transform.position - transform.position).normalized * 1000);
        fireTime = 0.0f;
        Debug.Log(_damage);
    }
